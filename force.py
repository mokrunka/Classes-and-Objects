...
def find_net_force(force_list):
    ...
    for force in force_list:
        horizontal_sum += force.get_horizontal()
        vertical_sum += force.get_vertical()

    magnitude = round(((horizontal_sum ** 2) + (vertical_sum ** 2)) ** 0.5, 1)
    angle = round(degrees(atan2(vertical_sum, horizontal_sum)), 1)

    res_force = Force(magnitude, angle)
    return res_force
...
#         self.vertical_sum = 0
#         for force in force_list:
#             self.horizontal_sum += force.get_horizontal()
#             self.vertical_sum += force.get_vertical()
#
#     def magnitude(self):
#         return round(((self.horizontal_sum ** 2) + (self.vertical_sum ** 2)) ** 0.5, 1)
#
#     def get_angle(self):
#         return round(degrees(atan2(self.vertical_sum, self.horizontal_sum)), 1)
...
# tests for second half of problem to sum forces and calculate an equivalent force
force_1 = Force(50, 90)
force_2 = Force(75, -90)
force_3 = Force(100, 0)
